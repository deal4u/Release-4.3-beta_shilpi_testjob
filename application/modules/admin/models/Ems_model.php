...
private $payment_processor;
	...
	public function existmodeSetting($mode){
		$chargemode = get_setting('payment_method','payment_mode','1',$mode);
		$authorize = (array)json_decode($chargemode[0]['meta_content']);
		$authorize['payment_mode'] = $mode;
		$authorize = array_merge($authorize, $this->authorize);
	...
	public function curlRequest($endpoint,$details){
		...
		if ($err) {		
			...
			if(isset($response_array['messages']['resultCode']) && $response_array['messages']['resultCode'] =='Error'){
				if(isset( $response_array['transactionResponse']['errors'][0]['errorText'])&&  $response_array['transactionResponse']['errors'][0]['errorText']!=""){
...
/* charge functionality on add customer */
	public function charge_customer($customer, $amount,$rep,$card_details,$renewal_id='') {
		...
		if(isset($response_array['msg']) && $response_array['msg']=="error"){
			...
			$this->save_payment($customer,$customer_details ,$response_array, $rep, $amount,$authorize['payment_mode'],$renewal_id);
			$finalResult = array('msg' => 'success', 'response' => "1",'charge_status' => '1' );
		}			
		return $finalResult;
    }

/* create profile of authorise while adding customer if cron is off */	
	public function createCustomerProfile($customer_id, $card_details= [], $policydata= []){
        ...
        $payment_profile= $this->customer_model->get_customer_payment_profile($customer_id);//get payment profile_id from db
		...
		if($response_array['messages']['resultCode'] == 'Error'){
			...
			$prifileentry = $this->customer_model->create_customer_payment_profile($customer_id,$authorize['payment_mode'], $result);
			sleep('15');
			$shippingaddress = 	'{
			...
			if(isset( $responseShipping['customerAddressId'])){
				...
				$this->customer_model->create_customer_payment_profile($customer_id,$authorize['payment_mode'], $result);
			}	
...
/* VOID TRANSACTION */
	public function void_payment($details, $rep){
		$authorize = $this->existmodeSetting($details['payment_mode']);
		$login_id = (isset($authorize['merchant_id'])) ? $authorize['merchant_id'] : '';
        $merchant_transaction_key = (isset($authorize['merchant_security'])) ? $authorize['merchant_security'] : '';
		$endpoint = $authorize['endpoint'];
        ...
        $voidResponse = $this->curlRequest($endpoint,$data);
        if (isset($voidResponse['messages']['resultCode']) && $voidResponse['messages']['resultCode'] == 'Ok') {
			$this->payments_model->insert_void($voidResponse ,$details, $rep);
			return $finalResult = array('msg' => 'success', 'response' => '1');
        } else {
			 return $finalResult = array('msg' => 'error', 'response' =>  'Something went Wrong.');
        }
    }
...
/* REFUND Payment */
	public function refund_payment($details, $amount, $rep){
				...
				"transactionType": "refundTransaction",
					...
					"payment": {
						"creditCard": {
							...
							"expirationDate": "'.$customer_card['card_exp_year'].'-'.sprintf("%02d", $customer_card['card_exp_month']).'"
						}
					},
					"refTransId": "'.$details['transaction_id'].'"
				}
		...
		$refundResponse = $this->curlRequest($endpoint,$data);
		if(isset($refundResponse['response']['transactionResponse']['errors'][0]['errorText']) && $refundResponse['response']['transactionResponse']['errors'][0]['errorText']!=""){
			$error = $refundResponse['response']['transactionResponse']['errors'][0]['errorText'];
			$finalResult = array('msg' => 'error', 'response' => $error);
		}else{
			$this->payments_model->insert_payment($refundResponse ,$details ,$amount, $rep);
			$finalResult = array('msg' => 'success', 'response' => $refundResponse['message'][0]['text'],'charge_status' => '1' );
		}
		return $finalResult;
	}
	
	public function updateArbCardDetails($customer_id,$customerprofile ,$policydata,$postdata){
		$customer =  get_data($customer_id,'customers');
		if(isset($customerprofile['payment_mode'])){
			$authorize = $this->existmodeSetting($customerprofile['payment_mode']);
		}else{
			$authorize = $this->modeSetting();
		}
        ...
        $endpoint = $authorize['endpoint'];
		$update_cc = 0;
		if(isset($customerprofile[0]['authorize']) && isset($customerprofile[0]['authorize_payment_profile'])){  ##if profile exists in db
			$customer_profile =  $customerprofile[0]['authorize'];
            $payment_profile_id =  $customerprofile[0]['authorize_payment_profile'];
            $details = '{
                "updateCustomerPaymentProfileRequest": {
                    "merchantAuthentication": {
                        "name": "'.$login_id.'",
                    ...
                    "paymentProfile": {
                        ...
                        "payment": {
                            "creditCard": {
                                "cardNumber": "'.$postdata['card_num'].'",
                                "expirationDate": "'.$postdata['card_year'].'-'.sprintf("%02d", $postdata['card_month']).'",
                                "cardCode": "'.$postdata['card_pin'].'",
                            }
                        },
                        "defaultPaymentProfile": false,
                        "customerPaymentProfileId": "'.$payment_profile_id.'"
		...
		if((isset($response['messages']['resultCode']) && $response['messages']['resultCode']=="Error") || (isset($response['msg']) && $response['msg'] =='error') ){
				return $response;
		}else{
			//$profileEntry = $this->customer_model->create_customer_payment_profile($customer_id,$this->payment_processor, $response); ##add entry in customer_payment_profiles table
		}
	}
	...
	public function charge_customerEMS($customer_id, $card_details, $policydata,$postdata){
		...
		$payment_profile= $this->customer_model->get_customer_payment_profile($customer_id);##get payment profile_id 
		...
		if(isset($result)){
			$profileEntry = $this->customer_model->create_customer_payment_profile($customer_id,$authorize['payment_mode'], $result); ##add entry in customer_payment_profiles table
	...
	public function shippingProfile($customer_id, $card_details, $policydata, $customer,$result,$postdata){
		...
		//if( isset($policydata['next_payment'] ) && $policydata['next_payment'] !=""){
			...
			$dataArray =  array(
				...
				'length'  =>  '1',//$policydata['latest_policy']['next_payment'],	
				'unit'  =>	"months",
				'startDate'  => $policydata['latest_policy']['next_bill_date'],	
				"totalOccurrences"=> $policydata['latest_policy']['charge_round'],	
				"trialOccurrences"=>  '0',
				"amount"=> $policydata['latest_policy']['next_payment']!=''?$policydata['latest_policy']['next_payment']:$policydata['latest_policy']['plan_total'],
				"trialAmount"=> "0.00",
			);
	...
	public function cancelSubscription($payment_profile, $policydata,$postdata){
		...
		if($response['messages']['message'][0]['code'] = 'I00002' ||$response['messages']['message'][0]['resultCode'] == 'Ok'){
			...
			$dataArray =  array(
				...
				'name'  => 'Subscription for '.$policydata['details']['email'],
				'length'  =>  '1', //$policydata['latest_policy']['payment_split'],	
				'unit'  =>	"months",
				'startDate'  => $policydata['latest_policy']['next_bill_date'],	
				"totalOccurrences"=> $policydata['latest_policy']['charge_round'],	
				"trialOccurrences"=>  '0',
	...
	public function updatesubscription($customer_id, $data){
	  //print_R($data);
        ...
        $payment_profile= $this->customer_model->get_customer_payment_profile($customer_id,$authorize['payment_mode']);
