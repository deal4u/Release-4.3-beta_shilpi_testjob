...
abstract class REST_Controller extends CI_Controller {
     ...
     * Fields: db, ignoreLimits, key, level, ownerId
     * Note: This is a dynamic object (stdClass)
     *
     * @var object
     */
    ...
    protected function _detect_api_key() {
        ...
        $this->rest->ownerId = NULL;
        $this->rest->ignoreLimits = FALSE;
        ...
        if (($key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $this->input->server($key_name))) {
            ...
            isset($row->ownerId) && $this->rest->ownerId = $row->ownerId;
            isset($row->level) && $this->rest->level = $row->level;
            isset($row->ignoreLimits) && $this->rest->ignoreLimits = $row->ignoreLimits;
             ...
             */
            if (empty($row->isPrivateKey) === FALSE) {
                // Check for a list of valid ip addresses
                if (isset($row->ipAddresses)) {
                    // multiple ip addresses must be separated using a comma, explode and loop
                    $list_ipAddresses = explode(',', $row->ipAddresses);
                    $found_address = FALSE;

                    foreach ($list_ipAddresses as $ip_address) {
    ...
    protected function _check_limit($controller_method) {
        ...
        if (empty($this->rest->ignoreLimits) === FALSE) {
            // Everything is fine
            return TRUE;
        }
